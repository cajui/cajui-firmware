...
ReadResult NvsBlob::read(uint8_t* out, size_t capacity, size_t& size) {
    ...
    auto error = nvs_get_blob(handle_, key_, nullptr, &size);
    if (error == ESP_ERR_NVS_NOT_FOUND) return ReadResult::Missing;
    if (error != ESP_OK || size > capacity) return ReadResult::Error;
    return nvs_get_blob(handle_, key_, out, &size) == ESP_OK ? ReadResult::Ok : ReadResult::Error;
}
bool NvsBlob::replace(const uint8_t* input, size_t size) {
    return opened_ && size >= minimum_ && size <= maximum_ &&
           nvs_set_blob(handle_, key_, input, size) == ESP_OK && nvs_commit(handle_) == ESP_OK;
}
}
#endif
