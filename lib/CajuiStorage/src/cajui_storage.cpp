...
struct Writer {
    ...
    void block(const uint8_t* src, size_t size) { std::memcpy(p, src, size); p += size; }
};
struct Reader {
    const uint8_t* p;
...
Binding PersistentStore::asBinding(const Entry& e, uint64_t network) const {
    Binding b{}; b.network = network; b.node = e.node; b.key = e.key; b.active = true;
    return b;
}
int PersistentStore::find(uint64_t node, uint64_t generation) const {
    for (size_t i = 0; i < BindingCapacity; ++i) {
        ...
        if (e.state != Enrollment::Empty && e.node == node && e.generation == generation) return int(i);
    }
    return -1;
}
...
void PersistentStore::encode() {
    ...
    for (size_t i = 0; i < next_.count; ++i) {
        ...
        w.number(q.entry, 1); w.number(q.frame.size, 2); w.block(q.frame.bytes.data(), MaxFrame);
    }
...
Result PersistentStore::prepare(uint64_t network, uint64_t receiver, uint64_t node,
    ...
    if (state_.network && (network != state_.network || receiver != state_.receiver || profile != state_.profile))
    ...
    for (size_t i = 0; i < BindingCapacity; ++i) {
        const auto& e = state_.entries[i];
        if (e.state == Enrollment::Empty) { if (empty < 0) empty = int(i); continue; }
        if (e.key == key || e.generation == generation ||
            (e.node == node && e.state == Enrollment::Prepared)) return Result::Conflict;
    }
    if (empty < 0) return Result::Full;
    next_ = state_; next_.network = network; next_.receiver = receiver; next_.profile = profile;
    auto& e = next_.entries[size_t(empty)]; e.state = Enrollment::Prepared;
    e.node = node; e.generation = generation; e.key = key;
...
bool PersistentStore::info(uint64_t node, uint64_t generation, EnrollmentInfo& out) const {
    ...
    out.state = e.state; out.node = e.node; out.generation = e.generation; out.counter = e.counter;
    return true;
}
...
bool PersistentStore::reserve(const Binding& b, uint64_t& out) {
    ...
    if (index < 0 || role_ != Role::Transmitter || state_.entries[size_t(index)].counter == UINT64_MAX)
        return false;
    next_ = state_; ++next_.entries[size_t(index)].counter;
...
Result PersistentStore::commit(const Binding& b, uint64_t expected, const Receipt& receipt, const Data&) {
    const int index = authorized(b);
    ...
    if (open(b, receipt.last, m) != Result::Ok || m.type != Type::Data ||
        m.counter != receipt.counter || m.counter <= expected) return Result::Invalid;
    if (state_.count == QueueCapacity) return Result::Full;
    next_ = state_;
    next_.entries[size_t(index)].receipt = receipt;
    auto& q = next_.queue[next_.count++]; q.entry = uint8_t(index); q.frame = receipt.last;
    return save() ? Result::Ok : Result::StorageError;
}
