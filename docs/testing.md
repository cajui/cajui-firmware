...
OpenSSL AES-GCM with ASan/UBSan. Journal and counter storage are explicit in-memory
test doubles, not flash simulations with a proven power-loss model.
...
Coverage gates apply only to `cajui_protocol.cpp` and the active host branch of
`crypto.cpp`: 95% lines and 85% branches. Compiler/library allocation failures are
