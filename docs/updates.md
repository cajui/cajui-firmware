...
or Edge over Web Serial. The manifests set `new_install_prompt_erase: false`: the installer
writes from offset 0 up to the end of the application and never erases the device. The
merged image stops well before `cajui`.
...
one; a local build (version 0) accepts any signed version. Samples keep queueing during the
...
operation (radio listening, storage mounted); the transmitter after its first complete
delivery cycle. A restart before that, from a crash, the watchdog or a fault, makes the
...
images must be signed with. The private key is a repository secret,
`FIRMWARE_SIGNING_KEY` (PEM), used only by the release job; nobody else can produce an
update the receiver accepts. Keep an offline backup: without it, boards can only be updated
