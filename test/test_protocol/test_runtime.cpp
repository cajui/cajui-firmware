...
struct Rig {
    ...
    void start() {
        TEST_ASSERT_EQUAL_INT(int(StartResult::Started),
                              int(controller.start(binding(), sample(), counter)));
    }
...
void completion(Completion expected, const Rig& rig, UNITY_LINE_TYPE line) {
    UNITY_TEST_ASSERT_EQUAL_INT(int(expected), int(rig.controller.report().completion), line,
                                nullptr);
...
void test_runtime_idle_and_success_reuse() {
    ...
    r.controller.poll();
    r.controller.cancel();
    TEST_ASSERT_EQUAL_UINT(0, r.radio.sleepCalls);
    r.start();
    TEST_ASSERT_EQUAL_UINT64(1, r.counter.last);
    TEST_ASSERT_EQUAL_INT(int(StartResult::Busy),
                          int(r.controller.start(binding(), sample(), r.counter)));
    r.enterChannelCheck();
    r.transmit();
...
void test_runtime_jitter_and_busy_channel_are_bounded() {
    ...
    r.clock.advance(1);
    r.controller.poll();
    r.radio.channel = ChannelStatus::Busy;
    r.controller.poll();
    ...
    r.clock.advance(9499);
    r.controller.poll(); // One ms remains: CAD may start, no TX.
    r.clock.advance(1);
    r.controller.poll();
...
void test_runtime_retries_identical_frames_then_exhausts() {
    ...
    r.controller.poll();
    TEST_ASSERT_TRUE(r.controller.active());
    r.clock.advance(1);
    r.controller.poll();
...
void test_runtime_lost_ack_replays_without_duplicate_commit() {
    ...
    r.transmit();
    Frame firstAck{};
    TEST_ASSERT_EQUAL_INT(int(Result::Ok),
                          int(receive(binding(), r.radio.sent[0], *store, firstAck)));
    r.retry(100);
    TEST_ASSERT_EQUAL_INT(int(Result::Duplicate),
                          int(receive(binding(), r.radio.sent[1], *store, r.radio.incoming)));
...
void test_runtime_storage_failures_never_produce_transmission_or_ack() {
    ...
    blob.failAfter = true;
    TEST_ASSERT_EQUAL_INT(int(StartResult::ProtocolRejected),
                          int(r.controller.start(binding(), sample(), *tx)));
    TEST_ASSERT_EQUAL_INT(int(Result::StorageError), int(r.controller.protocolResult()));
    r.controller.poll();
    TEST_ASSERT_EQUAL_UINT(0, r.radio.cadCalls);
    TEST_ASSERT_TRUE(r.radio.sent.empty());
    blob.failAfter = false;
    tx = fixtures::mounted(blob);
    TEST_ASSERT_EQUAL_INT(int(StartResult::Started),
                          int(r.controller.start(binding(), sample(), *tx)));
    r.enterChannelCheck();
    r.transmit();
    ...
    Frame ack{};
    TEST_ASSERT_EQUAL_INT(int(Result::StorageError),
                          int(receive(binding(), r.radio.sent[0], *rx, ack)));
...
void test_runtime_bad_acks_do_not_extend_the_window() {
    ...
    r.radio.incoming = good;
    r.radio.incoming.bytes[50] ^= 1;
    r.controller.poll();
    r.radio.incoming = r.radio.sent[0];
    r.controller.poll(); // Authenticated DATA is not ACK.
    r.radio.incoming = ackFor(fixtures::data(2));
    r.controller.poll(); // Wrong sample.
    r.radio.incoming = ackFor(fixtures::data(1, 3), binding(3));
    r.controller.poll(); // Wrong node.
    r.radio.incoming = ackFor(fixtures::data(1, 2, 2), binding(2, 2));
    r.controller.poll(); // Wrong key.
    Message wrong{};
    wrong.type = Type::Ack;
    wrong.counter = 1;
    ...
    r.radio.incoming.size = SIZE_MAX;
    r.controller.poll();
...
void test_runtime_persistence_latency_counts_toward_deadline() {
    Rig r;
    r.counter.clock = &r.clock;
    r.counter.latencyMs = 10000;
    r.start();
    r.controller.poll();
    COMPLETION(Completion::Deadline, r);
    TEST_ASSERT_EQUAL_UINT64(1, r.counter.last);
...
void test_runtime_stop_failure_preserves_confirmed_delivery() {
    ...
    TEST_ASSERT_EQUAL_INT(int(StartResult::RadioUnavailable),
                          int(r.controller.start(binding(), sample(), r.counter)));
    r.radio.sleepOk = true;
    TEST_ASSERT_TRUE(r.radio.sleep());
}
void test_runtime_deadline_overrides_queued_valid_ack() {
    SendPolicy p;
    p.cycleTimeoutMs = 4000;
    Rig r(p);
    r.start();
    r.enterChannelCheck();
    r.transmit();
...
void test_runtime_time_wrap_and_hardware_completion_timestamp() {
    Rig r;
    r.clock.time = UINT32_MAX - 100;
    r.start();
    r.enterChannelCheck();
    r.controller.poll(); // TX started before clock rollover.
    r.clock.advance(200);
    r.radio.completedAt = r.clock.time;
    r.clock.advance(1499);
    ...
    r.controller.poll();
    r.transmit();
    r.radio.incoming = ackFor(r.radio.sent[1]);
    r.controller.poll();
    COMPLETION(Completion::Acknowledged, r);
}
void test_runtime_pending_operations_have_timeouts() {
    for (int stage = 0; stage < 2; ++stage) {
        ...
        r.enterChannelCheck();
        if (stage == 0)
            r.radio.channel = ChannelStatus::Pending;
        else {
            r.controller.poll();
            r.radio.tx = TransmitStatus::Pending;
        }
        r.controller.poll();
        TEST_ASSERT_TRUE(r.controller.active());
        r.clock.advance(stage == 0 ? 1000 : 3000);
        r.controller.poll();
...
void test_runtime_sleep_failure_retains_frame_and_blocks_reuse() {
    ...
    Rig initial;
    initial.start();
    initial.radio.failOneSleep = true;
    initial.controller.poll();
...
void test_runtime_random_failure_or_out_of_range_stops() {
    for (int scenario = 0; scenario < 3; ++scenario) {
        ...
        r.controller.poll();
        COMPLETION(Completion::RandomError, r);
        TEST_ASSERT_TRUE(r.radio.sent.empty());
    }
}
void test_runtime_rejects_invalid_policies_and_samples_before_reserving() {
    ...
    SendPolicy p;
    p.secondBackoffMaxMs = p.cycleTimeoutMs;
    Rig bad(p);
    TEST_ASSERT_EQUAL_INT(int(StartResult::InvalidPolicy),
                          int(bad.controller.start(binding(), sample(), bad.counter)));
    Rig r;
    auto data = sample();
    data.count = 0;
    TEST_ASSERT_EQUAL_INT(int(StartResult::ProtocolRejected),
                          int(r.controller.start(binding(), data, r.counter)));
    TEST_ASSERT_EQUAL_INT(int(Result::Invalid), int(r.controller.protocolResult()));
    TEST_ASSERT_EQUAL_UINT64(0, r.counter.last);
    auto b = binding();
    b.active = false;
    TEST_ASSERT_EQUAL_INT(int(StartResult::ProtocolRejected),
                          int(r.controller.start(b, sample(), r.counter)));
...
void test_runtime_cancel_each_phase_consumes_counter_without_reuse() {
    for (int phase = 0; phase < 5; ++phase) {
        ...
        r.controller.cancel();
        COMPLETION(Completion::Cancelled, r);
        auto count = r.radio.sent.size();
        r.controller.poll();
        TEST_ASSERT_EQUAL_UINT(count, r.radio.sent.size());
        r.start();
        TEST_ASSERT_EQUAL_UINT64(2, r.counter.last);
        r.controller.cancel();
    }
