...
void test_receiver_commit_restart_duplicate_and_drain() {
    ...
    store.reset();
    store = mounted(blob, Role::Receiver);
    TEST_ASSERT_TRUE(store->healthy());
...
void test_commit_ambiguity_after_power_loss_is_recovered() {
    ...
    store.reset();
    store = mounted(blob, Role::Receiver);
    EXPECT_RESULT(Result::Duplicate, receive(binding(), data(1), *store, ack));
    TEST_ASSERT_EQUAL_UINT32(1, store->queued());
}
...
void test_semantically_invalid_snapshots_fail_closed_even_with_valid_crc() {
    ...
    for (const auto& mutation : invalid) {
        ...
        blob.bytes = good;
        blob.bytes[mutation.offset] = mutation.value;
        repairChecksum(blob);
        store = mounted(blob);
        TEST_ASSERT_FALSE_MESSAGE(store->healthy(), "Invalid snapshot accepted");
        store.reset();
    }
    blob.bytes = good;
...
void test_corrupt_receiver_receipts_and_queued_frames_are_rejected() {
    ...
    blob.bytes = good;
    auto later = data(2);
    std::memcpy(blob.bytes.data() + 231, later.bytes.data(), later.size);
    repairChecksum(blob);
    store = mounted(blob, Role::Receiver);
    TEST_ASSERT_FALSE(store->healthy());
    store.reset();
    blob.bytes = good;
    blob.bytes[92] = uint8_t(ack.size);
    std::memcpy(blob.bytes.data() + 93, ack.bytes.data(), ack.size);
    repairChecksum(blob);
    store = mounted(blob, Role::Receiver);
    TEST_ASSERT_FALSE(store->healthy());
}
...
void test_counter_revision_exhaustion_and_operation_guards() {
    ...
    TEST_ASSERT_EQUAL_UINT64(0, counter);
    store.reset();
    for (size_t i = 14; i < 22; ++i) blob.bytes[i] = 0xff;
    repairChecksum(blob);
    store = mounted(blob);
    TEST_ASSERT_TRUE(store->healthy());
    ...
    store.reset();
    MemoryBlob receiverBlob;
    store = mounted(receiverBlob, Role::Receiver);
...
void test_mount_reports_why_storage_is_unavailable() {
    ...
    store.reset(new PersistentStore(blob, Role::Transmitter, 3));
    store->mount();
    ...
    repairChecksum(blob);
    store = mounted(blob);
    EXPECT_HEALTH(Health::Format, *store);
    blob.bytes = good;
    blob.bytes[41] = 0;
    repairChecksum(blob);
    store = mounted(blob);
    ...
    blob.failRead = false;
    store.reset(new PersistentStore(blob, Role(3), 2));
